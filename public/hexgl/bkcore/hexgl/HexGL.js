...
bkcore.hexgl.HexGL = function(opts)
{
	...
	window.addEventListener('message', function(e) {
		if (e.data && e.data.type === 'hexgl_pause') {
			self.paused = true;
		} else if (e.data && e.data.type === 'hexgl_resume') {
			self.paused = false;
		} else if (e.data && e.data.type === 'hexgl_revive') {
			self.paused = false;
			self.gameplay.result = self.gameplay.results.NONE;
			self.gameplay.step = 4; // Back to playing
			self.components.shipControls.active = true;
			self.components.shipControls.destroyed = false;
			self.components.shipControls.mesh.visible = true;
			// A quick reset of ship rotation/position to the track center might be needed,
			// or we can just let them continue from where they crashed but give them a slight push.
			self.components.shipControls.movement.set(0,0,0);
			self.components.shipControls.speed = 2; // slight push
		}
	}, false);
}
